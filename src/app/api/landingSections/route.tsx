...
export async function GET(req: NextRequest) {
  ...
  const LANDING_SECTIONS_ID = process.env.LANDING_SECTIONS_ID!;
  ...
  const upstreamPath = `/tenant/collectionObjects?collectionId=${encodeURIComponent(LANDING_SECTIONS_ID)}`;
  ...
  try {
    ...
    const upstreamJson = (await res.json()) as LandingSectionData[];

    const flattened: LandingSection[] = upstreamJson
      .map(({ objectValue }) => {
