...
export async function GET(req: NextRequest) {
  ...
  const PRODUCT_SHOWCASE_ID = process.env.PRODUCT_SHOWCASE_ID!;
  ...
  const upstreamPath = `/tenant/collectionObjects?collectionId=${encodeURIComponent(PRODUCT_SHOWCASE_ID)}`;
  ...
  try {
    ...
    const upstreamJson = (await res.json()) as ProductShowcaseData[];

    const flattened: ProductShowcase[] = upstreamJson
      .map(({ objectValue }) => {
        ...
        return {
          ...
          ...(typeof imageKey === "string" && imageKey.length > 0
            ? {
                imageUrl: `https://${process.env.BUCKET_NAME}.s3.us-east-1.amazonaws.com/${imageKey}`,
              }
            : {}),
        };
    });
