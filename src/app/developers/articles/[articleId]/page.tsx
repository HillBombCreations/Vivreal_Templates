...
import { getArticleBySlug } from "@/lib/api/developers";
...
const ArticlePost = async (props: { params: { articleId: string } }) => {
  ...
  if (!article) {
      ...
      <>
          ...
          <Link href="/developers/articles" className="inline-flex items-center gap-1 mt-6 text-primary hover:underline">
  ...
  return (
    <>
      ...
      <main className="pt-24 md:pt-32 pb-20 md:pb-32 max-w-4xl mx-auto px-4 prose prose-primary prose-headings:font-display prose-headings:font-bold prose-headings:text-primary animate-fade-in">
        <Link href="/developers/articles" className="inline-flex items-center gap-1 mb-8 text-purple-700 hover:underline">
          <ArrowLeft size={16} /> Back to Article List
        </Link>
...
export const generateMetadata = async (props: { params: { articleId: string } }) => {
  ...
  return {
    ...
    openGraph: {
      title: article.title,
      description: article.description,
      url: `https://vivreal.io/developers/articles/${articleId}`,
