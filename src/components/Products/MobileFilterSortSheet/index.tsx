...
import type { MobileFilterSortSheetProps } from "@/types/Products";

export default function MobileFilterSortSheet({
  open,
  onOpenChange,
  sortKey,
  filterType,
  ...
  const [draftFilterGroupType, setDraftFilterGroupType] = useState(filterGroupType);
  ...
  useEffect(() => {
    ...
    setOpenGroups(() =>
      (groups || []).reduce((acc, g, idx) => {
        acc[g.key] = idx === 0 || g.key === firstKey;
        return acc;
        ...
        >
          <div className="px-5 pt-4 pb-3 border-b border-black/10 shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold tracking-tight">Filters & Sort</h3>
              </div>
              <button
                type="button"
          ...
          <div className="flex-1 overflow-auto px-5 py-4">
                          ...
                          {hasOptions ? (
                            <div className="mt-1 flex flex-wrap gap-2">
                              <button
                                ...
                                style={{
                                  borderColor:
                                    draftFilterGroupType === g.key && draftFilterValue === ""
                                      ? "var(--primary)"
                                      : "rgba(0,0,0,0.10)",
                            ...
                            <div className="text-[13px] text-black/50">No filters</div>
                          )}
                        </div>
                      ) : null}
                    </div>
            ...
            <div className="mt-4 rounded-2xl border border-black/10 bg-white/70">
              ...
              >
                <span className="text-[12px] font-semibold text-black/70">Sort</span>

                <span className="inline-flex items-center gap-2">
                  <span className="text-[12px] font-semibold text-black/60">
                    {sortOptions.find((o) => o.key === draftSort)?.label ?? "Featured"}
                  </span>
                  <ChevronDown
                    className={[
                      "h-4 w-4 text-black/50 transition-transform",
              ...
              {sortOpen ? (
                <div id="mobile-sort-panel" className="px-4 pb-4">
                    ...
                    {sortOptions.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          `}
                          style={{
                            borderColor: active ? "var(--primary)" : "rgba(0,0,0,0.10)",
          ...
          <div className="shrink-0 border-t border-black/10 px-5 py-4 bg-white/80 backdrop-blur">
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                disabled={loading}
                className="
                  h-11 rounded-2xl border border-black/10
                  bg-white/70 text-sm font-semibold
                  disabled:opacity-50
                "
              >
                Clear
              ...
              <button
                ...
                disabled={loading}
                className="
                  h-11 rounded-2xl text-sm font-semibold shadow-sm
                  transition active:scale-[0.99]
                  disabled:opacity-50
                "
